...
const { MEGABYTE_IN_BYTES, DEFAULT, REGEX } = require('../constants')
...
const createS3Bucket = async (bucketName) => {
    try{
        if(bucketName && !await bucketExists(bucketName)){
            ...
            logger.success(`Successfully created an S3 bucket with the name: ${bucketName}`);
        }
    } catch(err){
...
const isValidBucketName = (bucketName) => {
    return REGEX.BUCKET_NAME.test(bucketName);
}
